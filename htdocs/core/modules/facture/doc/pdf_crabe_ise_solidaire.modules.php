 ...
 */
class pdf_crabe_ise_solidaire extends ModelePDFFactures
{
    var $db;
    var $name;
    var $description;
	...
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		if ($deja_regle > 0 || $creditnoteamount > 0 || $depositsamount > 0 || !empty($object->array_options['options_prime']))
		{
			$index++;
			$pdf->SetTextColor(0,0,60);
			$pdf->SetFillColor(224,224,224);
			...
			$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
			$pdf->MultiCell($largcol2, $tab2_hl, price($resteapayer - (!empty($object->array_options['options_prime']) && $resteapayer > 0?$object->array_options['options_prime']:0), 0, $outputlangs), $useborder, 'R', 1);
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if (!empty($targetcontact->email))
		{
			...
			$pdf->SetTextColor(0,0,60);
			$pdf->SetFont('','', $default_font_size);
			$pdf->MultiCell($w, 3, $outputlangs->transnoentities("Email").": ".$outputlangs->convToOutputCharset($targetcontact->email), '', 'R');
			$pdf->SetFont('','', $default_font_size - 2);
		}
