 ...
 */
class pdf_azur_ise_solidaire extends ModelePDFPropales
{
	var $db;
	var $name;
	var $description;
		...
		if ($conf->propal->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				if (!empty($object->array_options['options_prime']))
				{
					...
					$pdf->Rect($this->marge_gauche, $tab_top-1, $this->page_largeur-$this->marge_gauche-$this->marge_droite, $height_prime+1);
				...
				if ($notetoshow)
				{
					$tab_top = 92 + $height_incoterms + $height_prime;
				...
				*/
				...
				// concat other pdf
				//Commented by Christophe Battarel$files = array(dol_buildpath('core/modules/propale/doc/mandat-CGI-conso.pdf'));
				//Commented by Christophe Battarel$this->concat($pdf, $files);
	...
	function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
		...
		if ( ! empty($extrafields->attribute_label))
		{
			...
			{
				if ($key == 'prime' || $key == 'oblige') continue;
				...
				$pdf->SetFont('','', $default_font_size - 2);
				$pdf->SetXY($posxval, $posy);
				$pdf->MultiCell(80, 4, $object->array_options['options_'.$key], 0, 'L');
	...
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		if (!empty($object->array_options['options_prime']))
		{
			...
			$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
			$pdf->MultiCell($largcol2, $tab2_hl, price($object->total_ttc - $object->array_options['options_prime'], 0, $outputlangs), $useborder, 'R', 1);
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if ($this->emetteur->logo)
		{
			if (is_readable($logo))
			{
			    $height=$this->pdf_getHeightForLogo($logo);
		...
		if (count($arrayidcontact) > 0)
		{
			...
			$targetcontact = $object->contact;
		}
		else
			$targetcontact = $object->thirdparty;

		if (! empty($targetcontact->phone) || ! empty($targetcontact->phone_pro) || ! empty($targetcontact->phone_mobile)  || ! empty($targetcontact->fax))
		{
			$phoneinfo = $outputlangs->transnoentities("Phone").": ";
			if (! empty($targetcontact->phone_pro)) $phoneinfo .= $outputlangs->convToOutputCharset($targetcontact->phone_pro);
			elseif (! empty($targetcontact->phone)) $phoneinfo .= $outputlangs->convToOutputCharset($targetcontact->phone);
			if ((!empty($targetcontact->phone_pro) || !empty($targetcontact->phone)) && (! empty($targetcontact->phone_mobile) || ! empty($targetcontact->fax))) $phoneinfo .= " / ";
			...
			$pdf->MultiCell(100, 3, $phoneinfo, '', 'R');
			$pdf->SetFont('','', $default_font_size - 2);
		}
		...
		if (!empty($targetcontact->email))
		{
			...
			$pdf->SetTextColor(0,0,60);
			$pdf->SetFont('','', $default_font_size);
			$pdf->MultiCell(100, 3, $outputlangs->transnoentities("Email").": ".$outputlangs->convToOutputCharset($targetcontact->email), '', 'R');
			$pdf->SetFont('','', $default_font_size - 2);
		}
		...
		if ($showaddress)
		{
			...
			elseif ($usecontact && strtoupper(dolGetFirstLastname($object->contact->firstname, $object->contact->lastname)) == strtoupper($object->contact->socname))
			{
				$carac_client_name= $outputlangs->convToOutputCharset($targetcontact->getFullName($outputlangs,1));
				$particulier = true;
			}
			else {
			...
			$carac_client=pdf_build_address($outputlangs,$this->emetteur,$object->thirdparty,($usecontact?$object->contact:''),$usecontact && !$particulier,'target');
	...
	function _signature_area(&$pdf, $object, $posy, $outputlangs)
	{
		...
		$pdf->MultiCell($largcol - 2, $tab_hl, "Le client", 0, 'L', 1);

		$pdf->SetXY($posx + 2, $tab_top + $tab_hl + 8);
		$pdf->MultiCell($largcol - 2, $tab_hl, "Le _______________ à _____________", 0, 'L', 1);
	...
	{
		global $conf;
		$height=(empty($conf->global->MAIN_DOCUMENTS_LOGO_HEIGHT)?25:$conf->global->MAIN_DOCUMENTS_LOGO_HEIGHT);
		$maxwidth=90;
